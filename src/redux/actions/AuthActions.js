...
export const updatePersonalInfo = (data) => {
    return async (dispatch) => {
        dispatch({ type: UPDATE_PERSONAL_INFO });
        
        const store = localStorage.getItem("store");
        
        const authToken = store && JSON.parse(store) ? JSON.parse(store).auth.authToken : "";
        ...
        axios.patch(apiEndpoints.auth.signUp, data, config).then(res => {
            if (res.status !== 200) {
