...
export default class EducationSection extends React.Component {
	...
	onSubmit(e) {
		if(this.state.proceed == true) {
			this.setState({agreeMessage:false })
		} else {
			this.setState({agreeMessage:true });
			e.preventDefault();
		}
	}
	...
	isAgreedToTerms(bool) {
		if(bool===true) {
			this.setState({ proceed: true, agreeMessage: false })
		} else {
		...
		return (
			<Form name="signup2" encType='application/json' onSubmit={this.props.onFormSubmitted}>
				<Grid>
					...
					<SkillsSection
						seededSkills={this.props.seededSkills}
						onChangeSkills={this.props.onChangeSkills}
						selectedSkills={this.props.selectedSkills}
					/>
					<BottomSection
						isAgreedToTerms={this.isAgreedToTerms.bind(this)}
						agreeMessage={this.state.agreeMessage}
					/>
				</Grid>
